...
async function saveEntry() {
    ...
    try {
        ...
        // Save the document
        const docRef = await addDoc(collection(db, "tax_data"), entryWithTimestamp);
...
async function getEntries() {
    try {
        const entriesRef = collection(db, "tax_data");
        ...
        const entries = querySnapshot.docs.map(doc => {
            ...
            // Ensure each document has the required structure
            return {
                id: doc.id,
                ...data,
                tradeName: data.tradeName || 'Unnamed Entry',
                address: data.address || '',
                genMngr: data.genMngr || '',
                ...
                otherFees: data.otherFees || getDefaultOtherFees(),
...
async function deleteEntry(docId) {
    try {
        await deleteDoc(doc(db, "tax_data", docId));
...
async function testDbAccess() {
    try {
        const entriesRef = collection(db, "tax_data");
        const querySnapshot = await getDocs(entriesRef);
        console.log("Direct DB access - documents count:", querySnapshot.size);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function openModal() {
        ...
        if (modal) {
          ...
          // Debug: List all elements with class "modal"
          console.log("Elements with class 'modal':", document.querySelectorAll('.modal'));
        }
      }
      
    // Modify the modal trigger event listener
    document.getElementById('addedData')?.addEventListener('click', async () => {
        console.log('Modal trigger clicked');
        await showModalWithEntries();
        openModal();
    });
